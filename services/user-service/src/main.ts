...
import {
  ...
  stopOutboxPublisher,
} from '@/messaging/event-publisher';
import { shutdownUserTelemetry } from './tracing';
...
const main = async () => {
  try {
    ...
    const shutdown = async () => {
      ...
      try {
        await shutdownUserTelemetry();
      } catch (error) {
        hasErrors = true;
        logger.error({ error }, 'error during shutdownUserTelemetry');
      }
